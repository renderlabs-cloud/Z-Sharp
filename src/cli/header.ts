...
export function success(data: SuccessData) {
	return chalk.white(
		'\n' +
		`Code compilation ${chalk.green('succeeded')} in ${chalk.green(data.time)}ms with ${((data.vulnerabilities == 0) ? chalk.green('0') : ((data.vulnerabilities < 5) ? chalk.yellow(data.vulnerabilities) : chalk.red(data.vulnerabilities)))} known vulnerabilities.`
	);
};
